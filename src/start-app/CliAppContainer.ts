...
export const actions = {
  updateManifests: createAction('update_manifest', (manifest: Record<string, AppManifest>) => ({ payload: manifest })),
  setManifests: createAction('set_manifests', (manifests: Record<string, AppManifest>) => ({ payload: manifests })),
};
...
export class CliAppContainer extends EventEmitter<AppContainerEvents> {
  #framework: Fusion<[AppModule]>;
  #manifests: FlowSubject<Record<string, AppManifest>, Actions>;

  #updateTask: Promise<void> & { state?: 'pending' | 'fulfilled' | 'rejected' } = Promise.resolve();
  ...
  get currentApp() {
    return this.#framework.modules.app.current?.state.manifest;
  }
  ...
  constructor(args: {
    ...
    /** legacy wrapper */
    const currentApp = new DistributedState<AppManifest | null>('AppContainer.currentApp', null, eventHub);
    const previousApps = new DistributedState<Record<string, AppManifest>>('AppContainer.previousApps', {}, eventHub);
  ...
  //   const source = new URL(`/app-bundle.js?v=${Date.now}`, uri);
  ...
  #update(): void {
    this.#updateTask = new Promise((resolve, reject) => {
      ...
      this.#framework.modules.app.getAllAppManifests().subscribe({
        ...
        next: (value) =>
          this.#manifests.next(actions.updateManifests(indexManifests(value as unknown as AppManifest[]))),
      });
    });
  }
}
